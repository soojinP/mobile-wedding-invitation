...
export default function CameraApp() {
  ...
  const drawFrame = (ctx, size, frame, done) => {
    ...
    img.onload = () => {
      ...
      if (f === 'polaroid') {
        ...
        ctx.save()
        ctx.beginPath()
        ctx.arc(size - pad - ms / 2 - 10, size - bottomPad / 2, ms / 2, 0, Math.PI * 2)
        ctx.clip()
        ctx.drawImage(img, size - pad - ms - 10, size - bottomPad / 2 - ms / 2, ms, ms)
        ctx.restore()
        ctx.strokeStyle = '#eee'
        ctx.lineWidth = 2
        ctx.beginPath()
  ...
  const heartPath = (ctx, cx, cy, r) => {
    ...
    ctx.bezierCurveTo(cx - r * 1.5, cy - r * 0.3, cx - r * 0.5, cy - r * 1.3, cx, cy - r * 0.5)
    ctx.bezierCurveTo(cx + r * 0.5, cy - r * 1.3, cx + r * 1.5, cy - r * 0.3, cx, cy + r * 0.7)
  }
      ...
      <div className="cam-result">
        ...
        <div className="cam-result-actions">
          <button className="cam-action-btn" onClick={retake}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M17.65 6.35A7.96 7.96 0 0012 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08A5.99 5.99 0 0112 18c-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z" fill="currentColor"/></svg>
            <span>다시 찍기</span>
          </button>
          <button className="cam-action-btn cam-save-btn" onClick={savePhoto}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z" fill="currentColor"/></svg>
            <span>저장</span>
          </button>
        </div>
      </div>
    ...
    <div className="cam-app">
      ...
      <div className="cam-controls">
        <button className="cam-flip-btn" onClick={flipCamera}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M20 5h-3.17L15 3H9L7.17 5H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm-8 13c-2.76 0-5-2.24-5-5h1.5l-2-3-2 3H6c0 3.31 2.69 6 6 6v-1zm4.5-3l2-3h-1.5c0-3.31-2.69-6-6-6v1c2.76 0 5 2.24 5 5h-1.5l2 3z" fill="currentColor"/></svg>
        </button>
...
function FramePreview({ frame }) {
    ...
    return (
      <>
        <div className="frame-preview-film top">
          <div className="film-holes">{Array(8).fill(0).map((_, i) => <span key={i} />)}</div>
  ...
  if (frame.id === 'polaroid') {
    return <div className="frame-preview-polaroid"><span>with CM & SJ</span></div>
  }
  if (frame.id === 'strip') {
    return (
